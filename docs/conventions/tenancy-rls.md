...
- `attendance_records` and `audit_log` were created as **partition-ready**
  tables from day one (partition key part of every PK/unique constraint —
  see the comment block above the `Tenant` model in `schema.prisma`), and
  the same RLS pattern applies to them: `audit_log`'s composite
  ...
  [attendance.md](./attendance.md)). **Phase 5.2 landed the actual
  `PARTITION BY RANGE` conversion** for both, plus `platform_audit_log` —
  RLS proven to hold identically on the resulting partitions (a GRANT on
  the parent does not even propagate to a partition addressed directly by
  name, so a partition ends up MORE locked down by default, not less). See
