...
| [`docs/conventions/incident-response-dr.md`](./docs/conventions/incident-response-dr.md)       | Incident response runbook (severity/roles, detection tied to real 5.4 alerts, a breach/notification checklist using the real 4.1 containment tooling), the DR plan (scenarios + honest RPO/RTO + rollback + ransomware, built entirely on 6.2's tested backup/restore + 5.2/5.3's seams), and five real automated chaos experiments under `apps/api/test/chaos/` — two of which found and fixed a real weakness. (6.4) |
...
- **6.2** — Security hardening + accessibility (WCAG) + backups/DR (Phase
  6's second slice, a re-verification/hardening pass, not new business
  functionality): real secure headers + a CORS allow-list replacing the
  previous wide-open `app.enableCors()`; field-encryption KEY ROTATION +
  a secrets-provider seam (`ENCRYPTION_KEY_PROVIDER`, `EnvSecretsProvider`/
  a documented `CloudSecretsProvider` seam) added to `EncryptionService`,
  fully backward-compatible with every value already encrypted in the
  database; OPTIONAL tenant-user MFA reusing platform's own TOTP util
  verbatim; a fresh re-verification of refresh-rotation, rate limiting,
  API-key/webhook/SSO scoping, and raw-SQL usage (no gaps found); ONE new
  exhaustive cross-tenant isolation suite walking every module from 0.x
  through 6.1 (63 tests, zero real leaks found); this repo's first-ever
  CI workflow (build/lint/test + dependency + secret scanning); real WCAG
  2.1 AA fixes across `apps/portal`/`apps/admin` (focus management, color
  contrast, status announcements, keyboard-operable custom controls) plus
  new axe-core automated coverage in both apps' Playwright suites; and an
  encrypted Postgres + object-storage backup mechanism with a restore
  drill ACTUALLY RUN twice against the live local stack, proving data
...
- **6.4** — Incident response, disaster recovery & chaos engineering
  (Phase 6's FINAL slice, and the final step of the whole build): an
  incident response runbook (SEV1–4 classification, roles, a detection
  table tying each real 5.4 Prometheus alert to the incident type it
  signals, mitigation via the REAL existing levers — 4.1's tenant
  suspend/resume and cross-tenant audit read, 0.10's circuit-breaker
  reset, 5.3's HPA/KEDA autoscaling — and a breach/notification checklist
  built on 6.1's own export/consent/processing-register machinery, with
  Pakistan named as the concrete first-client jurisdiction alongside a
  general GDPR-style reference, both explicitly flagged for real legal
  counsel before relying on any specific timeline); a DR plan (DB loss,
  region outage, ransomware, bad deploy, data corruption — each mapped to
  a procedure and an HONEST RPO/RTO) that reuses 6.2's already-tested
  encrypted backup/restore, 5.2's archival, and 5.3's regional-deployment
  seam ENTIRELY rather than reimplementing any of them, plus one new,
  genuinely locally-runnable verification script
  (`ops/backup/verify-latest-backup.sh` — validates a real backup
  artifact's structure via `pg_restore --list`, no scratch database
  needed, proven against a real artifact in this session). The real core:
  FIVE automated chaos experiments (`apps/api/test/chaos/`) against the
  live local stack, each asserting a concrete outcome, not a checklist —
  a genuinely killed-and-restarted Redis child process; DB connection-
  pool exhaustion re-verified against the real `P2028` code path 5.4
  found; a BullMQ worker "killed" mid-payroll-run (escaping
  `PayrollRunProcessor`'s own loop uncaught, not just one employee's
  computation failing) then retried with zero double-apply; a genuine
  concurrent flood proving the REAL `POST /auth/login` route (not just
  the demo stand-in 6.3's own flood test already covers) stays up; and a
  REAL `SIGTERM` sent to a spawned `node dist/main.js` OS process,
  proving in-flight-request drain and an untouched sibling instance
  staying up throughout — the one experiment in this whole build that
  exercises `main.ts`'s own signal handler as a genuinely separate
  process, not an in-process stand-in. **Two of the five found and fixed
  a REAL weakness — in two layers, not one**, mirroring exactly how 5.4
  found and fixed its own two bugs: `TenantRateLimitService.enforce` AND
  (found immediately after, by the SAME test, once the first fix exposed
  it) 5.1's `PermissionsCacheService.getContext` — both on the hot path of
  every tenant-scoped/authenticated request, before the DB transaction
  even opens — had no error handling around their Redis calls, so a
  genuinely unreachable Redis turned into an uncaught error (and, via
  ioredis's own default queue-and-wait reconnect behavior, potentially a
  long hang) rather than a clean response for the WHOLE app on every
  request. Fixed three ways: a new client-level `commandTimeout` on the
  one shared Redis connection (bounds every command uniformly), and both
  call sites now fail OPEN (served/computed-fresh, unmetered/uncached)
  with a new `hrm_redis_unavailable_fail_open_total` metric making the
  degradation a real, alertable signal rather than a silent gap. The
  DB-pool-exhaustion and BullMQ-resumability experiments confirmed their
  respective mechanisms already held — a fully valid, honestly-reported
  chaos outcome, not every experiment needing to find a bug to be worth
  running. See
...
- [x] **6.1** Data privacy & residency — data-subject export/erasure (a
      ...
      [`docs/conventions/privacy-residency.md`](./docs/conventions/privacy-residency.md).
- [x] **6.2** Security hardening + WCAG accessibility + backups — secure
      headers + a real CORS allow-list, field-encryption key rotation + a
      secrets-provider seam, optional tenant MFA (reusing platform's TOTP
      util), a fresh re-verification of the auth/API/rate-limiting surface
      (no gaps found), one new exhaustive cross-tenant isolation suite
      covering every module since 0.x, this repo's first CI workflow
      (build/lint/test + dependency/secret scanning), real WCAG 2.1 AA
...
- [x] **6.3** Edge security & DDoS (WAF/CDN) — provider-portable
      ...
      [`docs/conventions/edge-security.md`](./docs/conventions/edge-security.md).
- [x] **6.4** Incident response + chaos engineering — an incident response
      runbook (severity classification, roles, detection tied to real 5.4
      Prometheus alerts, a breach/notification checklist built on 6.1's
      privacy work and 4.1's containment tooling), a DR plan (DB
      loss/region outage/ransomware/bad deploy/data corruption, each with
      an honest RPO/RTO, entirely reusing 6.2's tested backup/restore and
      5.2/5.3's archival/regional seams — never reimplemented), a new
      locally-runnable backup-structure verification script
      (`ops/backup/verify-latest-backup.sh`), and five real, automated
      chaos experiments (`apps/api/test/chaos/`) against the live local
      stack — a genuinely killed/restarted Redis process, DB
      connection-pool exhaustion re-verified against the real P2024/P2028
      code paths, a BullMQ worker killed mid-payroll-run and retried with
      no double-apply, a real request flood proving `POST /auth/login`
      stays up, and a REAL `SIGTERM` sent to a spawned OS process proving
      graceful drain while a sibling instance keeps serving. Two of the
      five found and FIXED a real weakness, in two layers — unguarded
      Redis calls in both the per-tenant rate limiter AND the permissions
      cache, both on every authenticated request's hot path, which turned
      a Redis outage into a raw 500 (or a long hang) for the whole app;
      fixed via a client-level command timeout plus both call sites
      failing open, logged and metered. See
      [`docs/conventions/incident-response-dr.md`](./docs/conventions/incident-response-dr.md).

**PHASE 6 COMPLETE.** Data privacy & residency (6.1) + security hardening/
accessibility/backups (6.2) + edge security & DDoS (6.3) + incident
response, DR & chaos engineering (6.4) — the platform's full privacy,
security, edge, and operational-resilience layer.

**ALL PHASES 0 THROUGH 6 ARE NOW COMPLETE.** See the closing note at the
end of this file for a summary of the entire build.

## 7. Build complete — Phases 0 through 6

The entire planned build is now finished. Every phase shipped on top of
the one before it, on ONE codebase, with zero forking — the
non-negotiables in § 2 held throughout:

- **Phase 0 — the multi-tenant foundational chassis**: tenancy/RLS,
  tenant resolution, auth/RBAC, country packs, licensing (SaaS vs.
  lifetime on-prem), the generic workflow/approval engine, notifications,
  audit logging/custom fields/i18n, and the resilience chassis (rate
  limiting, circuit breakers, load shedding, connection-pool protection,
  idempotency, graceful shutdown) every later module inherits for free.
- **Phase 1 — Core HR**: Employee, Leave, Attendance, ESS/MSS
  (portal + mobile), and the analytics dashboard — the first sellable
  MVP.
- **Phase 2 — compensation & the employee lifecycle**: Payroll (the
  pack-driven CALCULATE/DELEGATE engine), Performance management,
  Recruitment/Onboarding/Offboarding, and the Admin/HR Console tying them
  all together on `apps/portal`.
- **Phase 3 — extend without forking**: Operations modules (Expenses,
  Assets, Helpdesk, Announcements), Learning & Development, and
  Integrations (webhooks, the versioned public API + API keys, adapter
  seams, SSO) — every tenant/vendor extension point is now additive
  configuration or a new DI binding, never a fork.
- **Phase 3.5 — the go-live/compliance layer**: the data migration &
  onboarding toolkit, Benefits administration, E-signatures, a real
  production-shaped Pakistan country pack, and statutory/government
  reporting — what a real first client actually needs to go live and stay
  compliant.
- **Phase 4 — the commercial layer**: the vendor super-admin console
  (`apps/admin`, platform identity/MFA, tenant lifecycle, impersonation),
  real SaaS billing via Stripe, and white-label/branding — the platform
  became a commercially operable product, SaaS and on-prem alike.
- **Phase 5 — the scale layer**: data-layer hardening (PgBouncer, a real
  read replica, tenant-scoped Redis caching), table partitioning +
  archival, horizontal scaling/Kubernetes/regional deployment, and
  observability + load testing (structured logging, metrics, tracing, and
  two real bugs found and fixed under genuine load) — proven, not
  assumed, against real multi-instance infrastructure.
- **Phase 6 — privacy, security, edge & operational resilience**: data
  privacy & residency (GDPR-style export/erasure, consent, residency
  enforcement), security hardening + WCAG accessibility + tested
  encrypted backups, edge security/DDoS posture (WAF/CDN config-as-code +
  the real app-side half), and — this final step — incident response,
  disaster recovery, and chaos engineering: a runbook, a DR plan built
  entirely on 6.2/5.2/5.3's own tested mechanisms, and five real chaos
  experiments against the live stack that found and fixed a genuine
  resilience gap rather than merely asserting the chassis was safe.

Every step in this build followed the same discipline: build on what
already exists rather than re-implement it, verify claims against real
local infrastructure rather than assume them, and state plainly —
per step, never blurred — what was proven locally versus what remains
honestly out of reach of this sandboxed environment (a real Kubernetes
cluster, a second region, a live Stripe/ACME account, real DNS). That
discipline is itself part of what got delivered, not just the code.
